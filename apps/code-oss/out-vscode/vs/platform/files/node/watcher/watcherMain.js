...
function parseWatcherPatterns(path, patterns, ignoreCase) {
  ...
  for (const pattern of patterns) {
    parsedPatterns.push(parse2(normalizeWatcherPattern(path, pattern), { ignoreCase }));
  }
  return parsedPatterns;
}
...
var BaseWatcher = class extends Disposable {
  ...
  onMonitoredPathAdded(request) {
    ...
    const event = { resource: URI.file(request.path), type: 1 /* ADDED */, cId: request.correlationId };
  ...
  traceEvent(event, request) {
    if (this.verboseLogging) {
      const traceMsg = ` >> normalized ${event.type === 1 /* ADDED */ ? "[ADDED]" : event.type === 2 /* DELETED */ ? "[DELETED]" : "[CHANGED]"} ${event.resource.fsPath}`;
...
var ParcelWatcher = class _ParcelWatcher extends BaseWatcher {
  ...
  handleIncludes(watcher, parcelEvents) {
    ...
    for (const { path, type: parcelEventType } of parcelEvents) {
      ...
      if (this.verboseLogging) {
        this.traceWithCorrelation(`${type === 1 /* ADDED */ ? "[ADDED]" : type === 2 /* DELETED */ ? "[DELETED]" : "[CHANGED]"} ${path}`, watcher.request);
      }
  ...
  filterEvents(events, watcher) {
    ...
    for (const event of events) {
      ...
      rootDeleted = event.type === 2 /* DELETED */ && isEqual(event.resource.fsPath, watcher.request.path, !isLinux);

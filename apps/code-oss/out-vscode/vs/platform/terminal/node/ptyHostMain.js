...
((Event8) => {
  ...
  function defer(event, flushOnListenerRemove, disposable) {
    return debounce2(event, () => void 0, 0, void 0, flushOnListenerRemove ?? true, void 0, disposable);
  }
  Event8.defer = defer;
  function once(event) {
  ...
  function onceIf(event, condition) {
    return Event8.once(Event8.filter(event, condition));
  }
  Event8.onceIf = onceIf;
  ...
  Event8.forEach = forEach;
  function filter(event, filter2, disposable) {
    return snapshot((listener, thisArgs = null, disposables) => event((e) => filter2(e) && listener.call(thisArgs, e), null, disposables), disposable);
  }
  Event8.filter = filter;
  function signal(event) {
    return event;
  }
  Event8.signal = signal;
  function any(...events) {
    return (listener, thisArgs = null, disposables) => {
  ...
  Event8.accumulate = accumulate;
  function throttle2(event, merge, delay = 100, leading = true, trailing = true, leakWarningThreshold, disposable) {
    let subscription;
    let output = void 0;
    let handle = void 0;
  ...
  function split(event, isT, disposable) {
    return [
      Event8.filter(event, isT, disposable),
      Event8.filter(event, (e) => !isT(e), disposable)
    ];
  }
  Event8.split = split;
  ...
  Event8.fromNodeEventEmitter = fromNodeEventEmitter;
  ...
  Event8.fromDOMEventEmitter = fromDOMEventEmitter;
  ...
  Event8.toPromise = toPromise;
  function forward(from, to) {
    return from((e) => to.fire(e));
  }
  Event8.forward = forward;
  function runAndSubscribe(event, handler, initial) {
    handler(initial);
    return event((e) => handler(e));
  }
  Event8.runAndSubscribe = runAndSubscribe;
  ...
  Event8.fromObservable = fromObservable;
  ...
  Event8.fromObservableLight = fromObservableLight;
})(Event || (Event = {}));
...
var AbstractMessageLogger = class extends AbstractLogger {
  ...
  trace(message, ...args) {
    if (this.canLog(1 /* Trace */)) {
      this.log(1 /* Trace */, format3([message, ...args], true));
    }
  }
  debug(message, ...args) {
    if (this.canLog(2 /* Debug */)) {
      this.log(2 /* Debug */, format3([message, ...args]));
    }
  }
  info(message, ...args) {
    if (this.canLog(3 /* Info */)) {
      this.log(3 /* Info */, format3([message, ...args]));
    }
  }
  warn(message, ...args) {
    if (this.canLog(4 /* Warning */)) {
      this.log(4 /* Warning */, format3([message, ...args]));
    }
  }
  error(message, ...args) {
    if (this.canLog(5 /* Error */)) {
      if (message instanceof Error) {
        ...
        this.log(5 /* Error */, format3(array));
      } else {
        this.log(5 /* Error */, format3([message, ...args]));
      }
    }
  }
  flush() {
...
function getSystemShellUnixLike(os4, env2) {
  if (isLinux && os4 === 2 /* Macintosh */ || isMacintosh && os4 === 3 /* Linux */) {
    return "/bin/bash";
  }
...
var RequestStore = class extends Disposable {
  /**
  ...
  constructor(collections) {
    ...
    collections.forEach((collection, extensionIdentifier) => {
      ...
      while (!next.done) {
        ...
        if (entry.length > 0 && entry[0].type === 1 /* Replace */) {
          next = it.next();
          continue;
        }
...
async function initWindowsVersionInfo() {
  ...
  if (!isWindows) {
    versionInfo = { release: os2.release(), buildNumber: 0 };
    return;
  }
  let buildNumber;
  let release2;
  try {
    ...
    const build = Registry2.GetStringRegKey("HKEY_LOCAL_MACHINE", versionKey, "CurrentBuild");
    if (build !== void 0) {
    ...
    const major = Registry2.GetDWORDRegKey("HKEY_LOCAL_MACHINE", versionKey, "CurrentMajorVersionNumber");
    const minor = Registry2.GetDWORDRegKey("HKEY_LOCAL_MACHINE", versionKey, "CurrentMinorVersionNumber");
    if (major !== void 0 && minor !== void 0 && build !== void 0) {
      release2 = `${major}.${minor}.${build}`;
    }
  } catch {
  } finally {
    versionInfo = {
      release: release2 || os2.release(),
...
async function getShellIntegrationInjection(shellLaunchConfig, options, env2, logService, productService, skipStickyBit = false) {
  if (!options.shellIntegration.enabled) {
    return { type: "failure", reason: "injectionSettingDisabled" /* InjectionSettingDisabled */ };
  }
  if (!shellLaunchConfig.executable) {
    return { type: "failure", reason: "noExecutable" /* NoExecutable */ };
  }
  if (shellLaunchConfig.isFeatureTerminal && !shellLaunchConfig.forceShellIntegration) {
    return { type: "failure", reason: "featureTerminal" /* FeatureTerminal */ };
  }
  if (shellLaunchConfig.ignoreShellIntegration) {
    return { type: "failure", reason: "ignoreShellIntegrationFlag" /* IgnoreShellIntegrationFlag */ };
  }
  const windowsBuildNumber = isWindows ? await getWindowsBuildNumberAsync() : 0;
  if (isWindows && windowsBuildNumber < 18309) {
    return { type: "failure", reason: "unsupportedWindowsBuild" /* UnsupportedWindowsBuild */ };
  }
  ...
  if (isWindows) {
    if (shell === "pwsh.exe" || shell === "powershell.exe") {
      ...
      if (!newArgs) {
        return { type: "failure", reason: "unsupportedArgs" /* UnsupportedArgs */ };
      }
      newArgs = [...newArgs];
    ...
    return { type: "failure", reason: "unsupportedShell" /* UnsupportedShell */ };
  }
  switch (shell) {
    case "bash": {
      if (!originalArgs || originalArgs.length === 0) {
    ...
    case "zsh": {
      ...
      const realTmpDir = realpathSync2(os3.tmpdir());
        ...
        try {
          ...
          if (err.message.includes("ENOENT")) {
            try {
              ...
              return { type: "failure", reason: "failedToCreateTmpDir" /* FailedToCreateTmpDir */ };
            }
            try {
              const chmodAsync = promisify2(chmod);
              await chmodAsync(zdotdir, 960);
              ...
              return { type: "failure", reason: "failedToSetStickyBit" /* FailedToSetStickyBit */ };
            }
          }
          logService.error(`Failed to set sticky bit on ${zdotdir}: ${err}`);
          return { type: "failure", reason: "failedToSetStickyBit" /* FailedToSetStickyBit */ };
...
var sensitiveEnvVarNames = /^(?:.*_)?(?:API_?KEY|TOKEN|SECRET|PASSWORD|PASSWD|PWD|CREDENTIAL|AUTH|PRIVATE_?KEY|ACCESS_?KEY|CLIENT_?SECRET|APIKEY)(?:_.*)?$/i;
...
function sanitizeEnvForLogging(env2) {
  if (!env2) {
    return env2;
  }
  const sanitized = {};
...
var JS_FILENAME_PATTERN = /[a-zA-Z-]+\.js\b/g;
...
var ChildProcessMonitor = class extends Disposable {
...
var WindowsShellHelper = class extends Disposable {
  ...
  getShellType(executable) {
    switch (executable.toLowerCase()) {
      case "cmd.exe":
        return "cmd" /* CommandPrompt */;
      case "powershell.exe":
      case "pwsh.exe":
        return "pwsh" /* PowerShell */;
      case "bash.exe":
      case "git-cmd.exe":
        return "gitbash" /* GitBash */;
      case "julia.exe":
        return "julia" /* Julia */;
      case "node.exe":
        return "node" /* Node */;
...
var TerminalProcess = class extends Disposable {
  constructor(shellLaunchConfig, cwd3, cols, rows, env2, _executableEnv, _options, _logService, _productService) {
    ...
    this._properties["initialCwd" /* InitialCwd */] = this._initialCwd;
    this._properties["cwd" /* Cwd */] = this._initialCwd;
    ...
    if (isWindows) {
      ...
      this._register(this.onProcessReady((e) => {
        ...
        this._register(this._windowsShellHelper.onShellTypeChanged((e2) => this._onDidChangeProperty.fire({ type: "shellType" /* ShellType */, value: e2 })));
        this._register(this._windowsShellHelper.onShellNameChanged((e2) => this._onDidChangeProperty.fire({ type: "title" /* Title */, value: e2 })));
      }));
    }
    this._register(toDisposable(() => {
  ...
  async start() {
    ...
    if (injection.type === "injection") {
      this._onDidChangeProperty.fire({ type: "usedShellIntegrationInjection" /* UsedShellIntegrationInjection */, value: true });
      ...
      this._onDidChangeProperty.fire({ type: "failedShellIntegrationActivation" /* FailedShellIntegrationActivation */, value: true });
      this._onDidChangeProperty.fire({ type: "shellIntegrationInjectionFailureReason" /* ShellIntegrationInjectionFailureReason */, value: injection.reason });
    ...
    try {
      ...
      if (errorMessage?.includes("Cannot launch conpty")) {
        return { message: localize(2397, null, "https://code.visualstudio.com/updates/v1_109#_removal-of-winpty-support", "terminal.integrated.windowsUseConptyDll") };
      }
  ...
  async _validateCwd() {
    try {
      ...
      if (!result.isDirectory()) {
        return { message: localize(2399, null, this._initialCwd.toString()) };
      }
    } catch (err) {
      if (err?.code === "ENOENT") {
        return { message: localize(2398, null, this._initialCwd.toString()) };
      }
    }
    this._onDidChangeProperty.fire({ type: "initialCwd" /* InitialCwd */, value: this._initialCwd });
    return void 0;
  }
  async _validateExecutable() {
    ...
    if (!executable) {
      return { message: localize(2400, null, slc.executable) };
    }
    try {
      const result = await fs2.promises.stat(executable);
      if (!result.isFile() && !result.isSymbolicLink()) {
        return { message: localize(2401, null, slc.executable) };
      }
  ...
  async setupPtyProcess(shellLaunchConfig, options, shellIntegrationInjection) {
    ...
    this._register(this._childProcessMonitor.onDidChangeHasChildProcesses((value) => this._onDidChangeProperty.fire({ type: "hasChildProcesses" /* HasChildProcesses */, value })));
    ...
    this._register(ptyProcess.onData((data) => {
      ...
      if (!this._isPtyPaused && this._unacknowledgedCharCount > 1e5 /* HighWatermarkChars */) {
        this._logService.trace(`Flow control: Pause (${this._unacknowledgedCharCount} > ${1e5 /* HighWatermarkChars */})`);
  ...
  _queueProcessExit() {
    if (this._logService.getLevel() === 1 /* Trace */) {
  ...
  _sendProcessTitle(ptyProcess) {
    ...
    this._onDidChangeProperty.fire({ type: "title" /* Title */, value: this._currentTitle });
    ...
    if (sanitizedTitle.toLowerCase().startsWith("python")) {
      this._onDidChangeProperty.fire({ type: "shellType" /* ShellType */, value: "python" /* Python */ });
    } else if (sanitizedTitle.toLowerCase().startsWith("julia")) {
      this._onDidChangeProperty.fire({ type: "shellType" /* ShellType */, value: "julia" /* Julia */ });
    } else {
      const shellTypeValue = posixShellTypeMap.get(sanitizedTitle) || generalShellTypeMap.get(sanitizedTitle);
      this._onDidChangeProperty.fire({ type: "shellType" /* ShellType */, value: shellTypeValue });
    }
  }
  shutdown(immediate) {
    if (this._logService.getLevel() === 1 /* Trace */) {
    ...
    switch (type) {
      case "cwd" /* Cwd */: {
        ...
        if (newCwd !== this._properties.cwd) {
          ...
          this._onDidChangeProperty.fire({ type: "cwd" /* Cwd */, value: this._properties.cwd });
        }
        return newCwd;
      }
      case "initialCwd" /* InitialCwd */: {
        const initialCwd = await this.getInitialCwd();
        if (initialCwd !== this._properties.initialCwd) {
          this._properties.initialCwd = initialCwd;
          this._onDidChangeProperty.fire({ type: "initialCwd" /* InitialCwd */, value: this._properties.initialCwd });
        }
        return initialCwd;
      }
      case "title" /* Title */:
  ...
  async updateProperty(type, value) {
    if (type === "fixedDimensions" /* FixedDimensions */) {
  ...
  acknowledgeDataEvent(charCount) {
    ...
    if (this._isPtyPaused && this._unacknowledgedCharCount < 5e3 /* LowWatermarkChars */) {
      this._logService.trace(`Flow control: Resume (${this._unacknowledgedCharCount} < ${5e3 /* LowWatermarkChars */})`);
...
var TerminalCapabilityStore = class extends Disposable {
  ...
  get onDidAddCommandDetectionCapability() {
    return Event.map(Event.filter(this.onDidAddCapability, (e) => e.id === 2 /* CommandDetection */, this._store), (e) => e.capability, this._store);
  }
  get onDidRemoveCommandDetectionCapability() {
    return Event.map(Event.filter(this.onDidRemoveCapability, (e) => e.id === 2 /* CommandDetection */, this._store), () => void 0, this._store);
  }
  get onDidAddCwdDetectionCapability() {
    return Event.map(Event.filter(this.onDidAddCapability, (e) => e.id === 0 /* CwdDetection */, this._store), (e) => e.capability, this._store);
  }
  get onDidRemoveCwdDetectionCapability() {
    return Event.map(Event.filter(this.onDidRemoveCapability, (e) => e.id === 0 /* CwdDetection */, this._store), () => void 0, this._store);
  }
  get items() {
    return this._map.keys();
  }
...
var PromptInputModel = class extends Disposable {
  ...
  _doSync() {
    ...
    if (this._shellType === "fish" /* Fish */ && (!line || !commandLine)) {
      commandStartY += 1;
    ...
    for (let y = commandStartY + 1; y <= absoluteCursorY; y++) {
      ...
      if (lineText && nextLine) {
        ...
        } else if (this._shellType === "fish" /* Fish */) {
          if (value.endsWith("\\")) {
...
var CommandDetectionCapability = class extends Disposable {
  constructor(_terminal, _logService) {
    super();
    this._terminal = _terminal;
    this._logService = _logService;
    this.type = 2 /* CommandDetection */;
    this._commands = [];
  ...
  _handleCursorMove() {
    ...
    if (this._terminal.buffer.active === this._terminal.buffer.normal && this._currentCommand.commandStartMarker) {
      if (this._terminal.buffer.active.baseY + this._terminal.buffer.active.cursorY < this._currentCommand.commandStartMarker.line) {
        ...
        this._onCurrentCommandInvalidated.fire({ reason: "windows" /* Windows */ });
      }
    }
  }
...
var WindowsPtyHeuristics = class extends Disposable {
  constructor(_terminal, _capability, _hooks, _logService) {
    super();
    this._terminal = _terminal;
    this._capability = _capability;
    ...
    this._register(this._capability.onBeforeCommandFinished((command) => {
      if (command.command.trim().toLowerCase() === "clear" || command.command.trim().toLowerCase() === "cls") {
        ...
        this._hooks.onCurrentCommandInvalidatedEmitter.fire({ reason: "windows" /* Windows */ });
      }
    }));
  }
  preHandleResize(e) {
...
var PartialCommandDetectionCapability = class extends DisposableStore {
  constructor(_terminal, _onDidExecuteText) {
    ...
    this.type = 3 /* PartialCommandDetection */;
    this._commands = [];
...
var BufferMarkCapability = class extends Disposable {
  constructor(_terminal) {
    super();
    this._terminal = _terminal;
    this.type = 4 /* BufferMarkDetection */;
...
var PromptTypeDetectionCapability = class extends Disposable {
  constructor() {
    super(...arguments);
    this.type = 6 /* PromptTypeDetection */;
...
var ShellIntegrationAddon = class extends Disposable {
  ...
  activate(xterm) {
    ...
    this.capabilities.add(3 /* PartialCommandDetection */, this._register(new PartialCommandDetectionCapability(this._terminal, this._onDidExecuteText)));
  ...
  _handleFinalTermSequence(data) {
    ...
    if (this._status === 0 /* Off */) {
      this._status = 1 /* FinalTerm */;
  ...
  _handleVSCodeSequence(data) {
    ...
    if (this._status !== 2 /* VSCode */) {
      this._status = 2 /* VSCode */;
      this._onDidChangeStatus.fire(this._status);
    }
    return didHandle;
  }
  async _ensureCapabilitiesOrAddFailureTelemetry() {
    ...
    this._activationTimeout = setTimeout(() => {
      if (!this.capabilities.get(2 /* CommandDetection */) && !this.capabilities.get(0 /* CwdDetection */)) {
      ...
      case "P" /* Property */: {
        ...
        switch (key) {
          ...
          case "Task": {
            ...
            this.capabilities.get(2 /* CommandDetection */)?.setIsCommandStorageDisabled();
            return true;
          }
        }
      }
  ...
  _updateCwd(value) {
    ...
    const commandDetection = this.capabilities.get(2 /* CommandDetection */);
  ...
  serialize() {
    if (!this._terminal || !this.capabilities.has(2 /* CommandDetection */)) {
      return {
        isWindowsPty: false,
  ...
  _createOrGetCwdDetection() {
    let cwdDetection = this.capabilities.get(0 /* CwdDetection */);
    if (!cwdDetection) {
      cwdDetection = this._register(new CwdDetectionCapability());
      this.capabilities.add(0 /* CwdDetection */, cwdDetection);
    }
    return cwdDetection;
  }
  _createOrGetCommandDetection(terminal) {
    let commandDetection = this.capabilities.get(2 /* CommandDetection */);
    if (!commandDetection) {
      commandDetection = this._register(new CommandDetectionCapability(terminal, this._logService));
      this.capabilities.add(2 /* CommandDetection */, commandDetection);
    }
    return commandDetection;
  }
  _createOrGetBufferMarkDetection(terminal) {
    let bufferMarkDetection = this.capabilities.get(4 /* BufferMarkDetection */);
    if (!bufferMarkDetection) {
      bufferMarkDetection = this._register(new BufferMarkCapability(terminal));
      this.capabilities.add(4 /* BufferMarkDetection */, bufferMarkDetection);
    }
    return bufferMarkDetection;
  }
  _createOrGetShellEnvDetection() {
    let shellEnvDetection = this.capabilities.get(5 /* ShellEnvDetection */);
    if (!shellEnvDetection) {
      shellEnvDetection = this._register(new ShellEnvDetectionCapability());
      this.capabilities.add(5 /* ShellEnvDetection */, shellEnvDetection);
    }
    return shellEnvDetection;
  }
  _createOrGetPromptTypeDetection() {
    let promptTypeDetection = this.capabilities.get(6 /* PromptTypeDetection */);
    if (!promptTypeDetection) {
      promptTypeDetection = this._register(new PromptTypeDetectionCapability());
      this.capabilities.add(6 /* PromptTypeDetection */, promptTypeDetection);
    }
...
var AutoRepliesPtyServiceContribution = class {
...
function traceRpc(_target, key, descriptor) {
  ...
  descriptor[fnKey] = async function(...args) {
    if (this.traceRpcArgs.logService.getLevel() === 1 /* Trace */) {
...
var PtyService = class extends Disposable {
  ...
  async _reviveTerminalProcess(workspaceId, terminal) {
    const restoreMessage = localize(2396, null);
    ...
    const newId = await this.createProcess(
      {
        ...
        name: terminal.processDetails.titleSource === 0 /* Api */ ? terminal.processDetails.title : void 0,
  ...
  async resize(id2, cols, rows, pixelWidth, pixelHeight) {
    const pty = this._throwIfNoPty(id2);
    if (pty) {
      for (const contrib of this._contributions) {
        contrib.handleProcessResize(id2, cols, rows, pixelWidth, pixelHeight);
      }
    ...
    if (direction === "win-to-unix") {
      ...
      return new Promise((c) => {
        const proc = execFile(wslExecutable, ["-e", "wslpath", original], {}, (error, stdout, stderr) => {
          c(error ? original : escapeNonWindowsPath(stdout.trim(), "bash" /* Bash */));
        });
        proc.stdin.end();
      });
    }
...
var PersistentTerminalProcess = class extends Disposable {
  ...
  async start() {
    ...
    this._onDidChangeProperty.fire({ type: "title" /* Title */, value: this._terminalProcess.currentTitle });
    this._onDidChangeProperty.fire({ type: "shellType" /* ShellType */, value: this._terminalProcess.shellType });
    this.triggerReplay();
    return void 0;
  }
  shutdown(immediate) {
...
async function startPtyHost() {
  ...
  if (_isUtilityProcess) {
    ...
    server = new Server("ptyHost" /* PtyHost */);
  }
  const productService = { _serviceBrand: void 0, ...product_default };
  const environmentService = new NativeEnvironmentService(parseArgs(process.argv, OPTIONS), productService);
  const loggerService = new LoggerService(getLogLevel(environmentService), environmentService.logsHome);
  server.registerChannel("logger" /* Logger */, new LoggerChannel(loggerService, () => DefaultURITransformer));
  const logger = loggerService.createLogger("ptyhost", { name: localize(2395, null) });
  ...
  server.registerChannel("heartbeat" /* Heartbeat */, ProxyChannel.fromService(heartbeatService, disposables));
  ...
  server.registerChannel("ptyHost" /* PtyHost */, ptyServiceChannel);
  if (_isUtilityProcess) {
    server.registerChannel("ptyHostWindow" /* PtyHostWindow */, ptyServiceChannel);
  }
  process.once("exit", () => {

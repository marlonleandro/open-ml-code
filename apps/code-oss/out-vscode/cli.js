...
var LANGUAGE_DEFAULT, _isWindows, _isMacintosh, _isLinux, _isLinuxSnap, _isNative, _isWeb, _isElectron, _isIOS, _isCI, _isMobile, _locale, _language, _platformLocale, _translationsConfigFile, _userAgent, $globalThis, nodeProcess, isElectronProcess, isElectronRenderer, _platform, isWindows2, isMacintosh, isLinux, isNative, isWeb, isWebWorker, webWorkerOrigin, userAgent, language, Language, setTimeout0IsFaster, setTimeout0, isChrome, isFirefox, isSafari, isEdge, isAndroid;
  ...
  "src/vs/base/common/map.ts"() {
    ...
    LinkedMap = class {
      ...
      touch(item, touch2) {
        ...
        if (touch2 !== 1 /* AsOld */ && touch2 !== 2 /* AsNew */) {
          return;
        }
        if (touch2 === 1 /* AsOld */) {
          if (item === this._head) {
...
var CSI_SEQUENCE, OSC_SEQUENCE, ESC_SEQUENCE, CONTROL_SEQUENCES, UTF8_BOM_CHARACTER, GraphemeBreakTree, AmbiguousCharacters, InvisibleCharacters;
...
function isRootOrDriveLetter(path2) {
  ...
  if (isWindows2) {
    ...
    return hasDriveLetter(pathNormalized) && (path2.length === 2 || pathNormalized.charCodeAt(2) === 92 /* Backslash */);
  }
  return pathNormalized === posix.sep;
}
function hasDriveLetter(path2, isWindowsOS = isWindows2) {
  if (isWindowsOS) {
    return isWindowsDriveLetter(path2.charCodeAt(0)) && path2.charCodeAt(1) === 58 /* Colon */;
  }
  return false;
}
    ...
    ExtUri = class {
      ...
      dirname(resource) {
        ...
        if (resource.scheme === Schemas.file) {
          ...
          if (resource.authority && dirname4.length && dirname4.charCodeAt(0) !== 47 /* Slash */) {
...
var Limiter, Queue, ResourceQueue, RunOnceScheduler, RunOnceWorker, ThrottledWorker, runWhenGlobalIdle, _runWhenIdle, DeferredPromise, Promises, AsyncIterableObject, ProducerConsumer, AsyncIterableProducer;
...
async function rimraf(path2, mode = 0 /* UNLINK */, moveToPath) {
...
var SymlinkSupport, writeQueues, canFlush, COPY_MODE_MASK, Promises2;
var init_pfs = __esm({
  "src/vs/base/node/pfs.ts"() {
    init_async();
    init_extpath();
...
var IFileService, FileChangesEvent, ByteSize;
...
function parseWatcherPatterns(path2, patterns, ignoreCase) {
  const parsedPatterns = [];
  for (const pattern of patterns) {
    parsedPatterns.push(parse2(normalizeWatcherPattern(path2, pattern), { ignoreCase }));
  }
  return parsedPatterns;
}
function isFiltered(event, filter) {
  if (typeof filter === "number") {
    switch (event.type) {
      case 1 /* ADDED */:
        return (filter & 4 /* ADDED */) === 0;
      case 2 /* DELETED */:
        return (filter & 8 /* DELETED */) === 0;
      case 0 /* UPDATED */:
        return (filter & 2 /* UPDATED */) === 0;
    }
  }
  return false;
}
    ...
    EventCoalescer = class {
      ...
      processEvent(event) {
        ...
        if (existingEvent) {
          ...
          if (existingEvent.resource.fsPath !== event.resource.fsPath && (event.type === 2 /* DELETED */ || event.type === 1 /* ADDED */)) {
            keepEvent = true;
          } else if (currentChangeType === 1 /* ADDED */ && newChangeType === 2 /* DELETED */) {
  ...
  "src/vs/platform/files/node/watcher/nodejs/nodejsWatcherLib.ts"() {
    ...
    NodeJSFileWatcherLibrary = class _NodeJSFileWatcherLibrary extends Disposable {
            ...
            if (isDirectory) {
              ...
              if (type === "rename" || !isEqual(changedFileName, pathBasename, !isLinux)) {
                const timeoutHandle = setTimeout(async () => {
                  ...
                  if (fileExists) {
                    this.onFileChange(
                      { resource: requestResource, type: 0 /* UPDATED */, cId: this.request.correlationId },
                      true
                      /* skip excludes/includes (file is explicitly watched) */
                    );
      ...
      onWatchedPathDeleted(resource) {
        ...
        this.onFileChange(
          { resource, type: 2 /* DELETED */, cId: this.request.correlationId },
          true
          /* skip excludes/includes (file is explicitly watched) */
        );
      ...
      onFileChange(event, skipIncludeExcludeChecks = false) {
        ...
        if (this.verboseLogging) {
          this.traceWithCorrelation(`${event.type === 1 /* ADDED */ ? "[ADDED]" : event.type === 2 /* DELETED */ ? "[DELETED]" : "[CHANGED]"} ${event.resource.fsPath}`);
        }
      ...
      handleFileChanges(fileChanges) {
        ...
        if (this.verboseLogging) {
          for (const event of filteredEvents) {
            this.traceWithCorrelation(` >> normalized ${event.type === 1 /* ADDED */ ? "[ADDED]" : event.type === 2 /* DELETED */ ? "[DELETED]" : "[CHANGED]"} ${event.resource.fsPath}`);
          }
        }
...
function buildHelpMessage(productName, executableName, version, options, capabilities) {
  ...
  const inputFiles = capabilities?.noInputFiles ? "" : capabilities?.isChat ? ` [${localize(1944, null)}]` : ` [${localize(1971, null)}...]`;
  const subcommand = capabilities?.isChat ? " chat" : "";
  const help = [`${productName} ${version}`];
  help.push("");
  help.push(`${localize(1991, null)}: ${executableName}${subcommand} [${localize(1969, null)}]${inputFiles}`);
  help.push("");
...
function buildStdinMessage(executableName, isChat) {
  ...
  return localize(1981, null, example);
}
function buildVersionMessage(version, commit) {
  return `${version || localize(1989, null)}
${commit || localize(1988, null)}
${process.arch}`;
}
...
function parseAndValidate(cmdLineArgs, reportWarnings) {
  const onMultipleValues = (id2, val) => {
    console.warn(localize(1999, null, id2, val));
  };
  const onEmptyValue = (id2) => {
    console.warn(localize(1997, null, id2));
  };
  const onDeprecatedOption = (deprecatedOption, message) => {
    console.warn(localize(1996, null, deprecatedOption, message));
  };
  const getSubcommandReporter = (command) => ({
    onUnknownOption: (id2) => {
      if (!NATIVE_CLI_COMMANDS.includes(command)) {
        console.warn(localize(2001, null, id2, command));
      }
    },
    onMultipleValues,
    onEmptyValue,
  ...
  const errorReporter = {
    onUnknownOption: (id2) => {
      console.warn(localize(2000, null, id2));
    },
    onMultipleValues,
    onEmptyValue,
    onDeprecatedOption,
  ...
  if (args.goto) {
    args._.forEach((arg) => assert2(/^(\w:)?[^:]+(:\d*){0,2}:?$/.test(arg), localize(1998, null)));
  }
  return args;
}
...
import { chmodSync, existsSync as existsSync2, readFileSync, statSync, truncateSync, unlinkSync, promises as promises6 } from "fs";
...
async function main(argv) {
  ...
  if (args.help) {
    ...
    if (!isMacintosh) {
      ...
      if (args.sessions) {
        const contentsPath = dirname2(dirname2(process.execPath));
        const applicationsPath = join2(contentsPath, "Applications");
        try {
          const files = await promises6.readdir(applicationsPath);
          const embeddedApp = files.find((file) => file.endsWith(".app"));
          if (embeddedApp) {
            appToLaunch = join2(applicationsPath, embeddedApp);
...
function configurePortable(product3) {
  ...
  function getApplicationPath() {
    ...
    if (process.platform === "win32" && product3.win32VersionedUpdate) {
      return path.dirname(path.dirname(path.dirname(appRoot)));
    }
...
if (process.isEmbeddedApp) {
  try {
    const productSubObj = require3("../product.sub.json");
    productObj = Object.assign(productObj, productSubObj);
  } catch (error) {
  }
  try {
    const pkgSubObj = require3("../package.sub.json");
